...
async function initDb() {
  ...
  `);
  // best-effort migration: add columns if they don't exist yet on an older DB
  try { await db.execute('ALTER TABLE cards ADD COLUMN client_id TEXT'); } catch (e) {}
  try { await db.execute('ALTER TABLE cards ADD COLUMN review_url TEXT'); } catch (e) {}
  try { await db.execute('ALTER TABLE cards ADD COLUMN platform TEXT'); } catch (e) {}
}
...
app.get('/r/:cardId', async (req, res) => {
  ...
  try {
    ...
    const targetUrl = card.review_url || GOOGLE_REVIEW_URL_DEFAULT;
...
app.post('/api/clients', authenticate, async (req, res) => {
  ...
  try {
    await db.execute({
      sql: 'INSERT INTO clients (id, name, password, created_at) VALUES (?, ?, ?, ?)',
      args: [id, name, password, new Date().toISOString()]
    });
...
app.get('/api/clients', authenticate, async (req, res) => {
  if (!req.scope.isMaster) return res.status(403).json({ error: 'master access only' });
  const result = await db.execute('SELECT id, name, password, created_at FROM clients ORDER BY created_at DESC');
...
app.post('/api/cards', authenticate, async (req, res) => {
  const { id, label, reviewUrl, platform } = req.body;
  if (!id || !label) return res.status(400).json({ error: 'id and label required' });
  if (!reviewUrl) return res.status(400).json({ error: 'reviewUrl required' });
  ...
  try {
    await db.execute({
      sql: 'INSERT INTO cards (id, label, review_url, platform, client_id, created_at) VALUES (?, ?, ?, ?, ?, ?)',
      args: [id, label, reviewUrl, platform || 'Review', clientId, new Date().toISOString()]
    });
    res.json({ ok: true });
  } catch (err) {
    console.error(err);
